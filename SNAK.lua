...
local NSNAK = io.open("NSNAK", 'w')
NSNAK:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
sudo lua5.3 NSNAK.lua
done
]])
NSNAK:close()
...
cd $(cd $(dirname $0); pwd)
while(true) do
screen -S NSNAK -X kill
screen -S NSNAK ./NSNAK
done
]])
Run:close()
Redis:del(SshId.."Info:Redis:User:ID");Redis:del(SshId.."Info:Redis:User");Redis:del(SshId.."Info:Redis:Token:User");Redis:del(SshId.."Info:Redis:Token")
os.execute('chmod +x NSNAK;chmod +x Run;./Run')
end
...
NSNAK = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..NSNAK)
LUATELE = URL.escape(""..SNAK.."\n"..UserBot.."\n"..UserSudo.."\n"..Token.."")
function var(value)  
...
--download('https://raw.githubusercontent.com/AhmedElsayed111/SNAK/master/SNAK.lua','SNAK.lua')
...
if text == (Redis:get(SNAK.."SNAK:Name:Bot") or "السلطان") then
local NamesBot = (Redis:get(SNAK.."SNAK:Name:Bot") or "السلطان")
local NameBots = {
"عمر "..NamesBot.. " شتريد؟",
"أჂ̤ أჂ̤ هياتني اني",
"موجود بس لتصيح",
...
local CmdStart = '*\n⌔︙أهلآ بك في بوت '..(Redis:get(SNAK.."SNAK:Name:Bot") or "السلطان")..
